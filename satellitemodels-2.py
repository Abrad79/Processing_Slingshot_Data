...
def convert_to_cartesian(
    ...
    """
    ix, iy, iz = spherical_to_unit(
        np.deg2rad(phi_i), np.deg2rad(theta_i)
    )
    ox, oy, oz = spherical_to_unit(
...
def aluminum_brdf(
    ...
    ix, iy, iz = w_i
    ox, oy, oz = w_o
    phi_i, theta_i = unit_to_spherical(ix, iy, iz)
    phi_o, theta_o = unit_to_spherical(ox, oy, oz)
...
def get_surfaces() -> list:
    ...
    SURFACES_LAB_BRDFS = [
        ...
        Surface(104.96,solar_array_normal,lab_solar_array_brdf),
    ]
